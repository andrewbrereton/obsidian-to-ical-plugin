...
export class IcalService {
  getCalendar(tasks: Task[]): string {
    ...
    let calendar =
      ...
      'BEGIN:VCALENDAR\r\n' +
      'VERSION:2.0\r\n' +
      'PRODID:-//Andrew Brereton//obsidian-ical-plugin v1.20.0//EN\r\n' +
  ...
  private getEvent(
    ...
    let event =
      ...
      '\r\n' +
      'DTSTAMP:' +
      task.getDate(null, 'YYYYMMDDTHHmmss') +
      '\r\n'
      ...
      switch (settings.howToProcessMultipleDates) {
        ...
        case 'PreferStartDate':
          if (task.hasA(TaskDateName.Start)) {
            ...
            task.hasA(TaskDateName.TimeStart) &&
            task.hasA(TaskDateName.TimeEnd)
          ) {
            event +=
              'DTSTART:' +
              task.getDate(TaskDateName.TimeStart, 'YYYYMMDD[T]HHmmss[Z]') +
              '\r\n'
            event +=
              'DTEND:' +
              task.getDate(TaskDateName.TimeEnd, 'YYYYMMDD[T]HHmmss[Z]') +
              '\r\n'
          } else {
        ...
        case 'CreateMultipleEvents':
          ...
          if (task.hasA(TaskDateName.Start)) {
            event += this.getEvent(
              task,
              task.getDate(TaskDateName.Start, 'YYYYMMDD'),
              '🛫 '
            )
          }
          ...
          if (task.hasA(TaskDateName.Scheduled)) {
            event += this.getEvent(
              task,
              task.getDate(TaskDateName.Scheduled, 'YYYYMMDD'),
              '⏳ '
            )
          }
          ...
          if (task.hasA(TaskDateName.Due)) {
            event += this.getEvent(
              task,
              task.getDate(TaskDateName.Due, 'YYYYMMDD'),
              '📅 '
            )
          }
        ...
        default:
          if (task.hasA(TaskDateName.Start) && task.hasA(TaskDateName.Due)) {
            event +=
              '' +
              'DTSTART:' +
              task.getDate(TaskDateName.Start, 'YYYYMMDDTHHmmss') +
              '\r\n' +
              'DTEND:' +
              task.getDate(TaskDateName.Due, 'YYYYMMDDTHHmmss') +
              '\r\n'
          } else if (task.hasA(TaskDateName.Due)) {
            event +=
              '' +
  ...
  private getToDos(tasks: Task[]): string {
    return tasks
      .map((task: Task) => {
        if (
          settings.isOnlyTasksWithoutDatesAreTodos &&
          task.hasAnyDate() === true
        ) {
  ...
  private getToDo(task: Task): string {
    ...
    if (task.hasA(TaskDateName.Due)) {
      toDo +=
        'DUE;VALUE=DATE:' + task.getDate(TaskDateName.Due, 'YYYYMMDD') + '\r\n'
    }

    if (task.hasA(TaskDateName.Done)) {
      toDo +=
        'COMPLETED;VALUE=DATE:' +
        task.getDate(TaskDateName.Done, 'YYYYMMDD') +
        '\r\n'
    }
