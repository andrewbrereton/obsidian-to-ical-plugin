...
export class SettingTab extends PluginSettingTab {
  ...
  async display(): Promise<void> {
    ...
    containerEl.createEl('p', {
      ...
      text: 'This plugin finds all of the tasks in your vault that contain a date and generates a calendar in iCalendar format. The calendar can be saved to a file and/or saved in a Gist on GitHub so that it can be added to your iCalendar calendar of choice.',
    })

    new Setting(containerEl)
      .setName('Processing internal links')
      .setDesc(
        'How should [[wikilinks]] and [markdown links](markdown links) be processed if they are encountered in a task?'
      )
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        'Choose if you want your calendar to ignore tasks that have been completed.'
      )
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        'As well as adding your tasks as calendar events, you can choose to add your tasks as todo items to your calendar'
      )
      .addToggle((toggle: ToggleComponent) =>
        toggle.setValue(settings.isIncludeTodos).onChange(async (value) => {
    ...
    if (settings.isIncludeTodos) {
      new Setting(containerEl)
        ...
        .setDesc(
          'When adding the TODO items to your iCalendar, should we only consider tasks without dates as TODO items?'
        )
        .addToggle((toggle: ToggleComponent) =>
          toggle
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        'Do you want to exclude tasks if they are older than a certain age? This could be useful if you have a very large number of tasks and are not interested in the past.'
      )
      .addToggle((toggle: ToggleComponent) =>
        toggle.setValue(settings.ignoreOldTasks).onChange(async (value) => {
    ...
    if (settings.ignoreOldTasks) {
      new Setting(containerEl)
        ...
        .setDesc(
          'If every date for a given task is more than this many days ago then it will be excluded from your calendar.'
        )
        .addText((text) =>
          text
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        'A task can have many dates (eg: due, start, scheduled, etc). When multiple dates are encountered in a task, which date do you want to use for the calendar?'
      )
      .addDropdown((dropdown: DropdownComponent) =>
        dropdown
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        'Turn this on if you want to support the Day Planner plugin format.'
      )
      .addToggle((toggle: ToggleComponent) =>
        toggle
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        'Do you want your calendar to only include tasks that contain certain tags?'
      )
      .addToggle((toggle: ToggleComponent) =>
        toggle
    ...
    if (settings.isIncludeTasksWithTags) {
      new Setting(containerEl)
        ...
        .setDesc(
          'Enter one or more tags. Separate multiple tags with a space. If one or more of these tags are found then the task will be included in your calendar.'
        )
        .addText((text) =>
          text
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        'Do you want your calendar to exclude tasks that contain certain tags?'
      )
      .addToggle((toggle: ToggleComponent) =>
        toggle
    ...
    if (settings.isExcludeTasksWithTags) {
      new Setting(containerEl)
        ...
        .setDesc(
          'Enter one or more tags. Separate multiple tags with a space. If one or more of these tags are found then the task will be excluded from your calendar.'
        )
        .addText((text) =>
          text
    ...
    if (settings.isSaveToGistEnabled) {
      ...
      containerEl.createEl('p', {
        cls: 'setting-item-description',
        text: 'Perform the following steps to get your Personal Access Token and Gist ID:',
      })
      const ol = containerEl.createEl('ol')
      ol.createEl('li', {
        cls: 'setting-item-description',
        text: 'Go to https://github.com/settings/tokens/new and create a new personal access token. It only needs Gist scope.',
      })
      ol.createEl('li', {
        cls: 'setting-item-description',
        text: 'Go to https://gist.github.com/ and create a new secret Gist.',
      })
      ...
      const githubPersonalAccessTokenErrorElement = containerEl.createEl('p', {
        text: '',
        cls: ['setting-message', 'cm-negative'],
      })
      ...
      new Setting(containerEl)
        ...
        .addText((text: TextComponent) =>
          text.setValue(settings.githubUsername).onChange(async (value) => {
    ...
    if (settings.isSaveToFileEnabled) {
      ...
      new Setting(containerEl)
        .setName('Path')
        .setDesc(
          'Which directory/folder do you want to save your calendar to? An empty string means to the current vault root path. The path must be inside the vault.'
        )
        .addText((text: TextComponent) =>
          text.setValue(settings.savePath).onChange(async (value) => {
      ...
      new Setting(containerEl)
        .setName('Filename')
        .setDesc(
          'What do you want to call the file of your calendar? An empty string means ' +
      ...
      new Setting(containerEl)
        ...
        .setDesc(
          'The file extension must be one of .ical or .ics or .ifb or .icalendar'
        )
        .addDropdown((dropdown: DropdownComponent) =>
          dropdown
            .addOptions({
    ...
    new Setting(containerEl)
      ...
      .addToggle((toggle: ToggleComponent) =>
        toggle.setValue(settings.isDebug).onChange(async (value) => {
  ...
  validateGithubPersonalAccessToken(value: string): void {
    ...
    const githubFineGrainedPersonalAccessTokenRegex =
      /^github_pat_[a-zA-Z0-9]{22}_[a-zA-Z0-9]{59}$/

    if (
      new RegExp(githubClassicPersonalAccessTokenRegex).test(value) ||
      new RegExp(githubFineGrainedPersonalAccessTokenRegex).test(value)
    ) {
      return
    }

    throw new Error(
      'GitHub Personal Access Token must start in "ghp_" for classic tokens or "github_pat_" for fine-grained tokens.'
    )
  }
