...
export class TaskFinder {
  ...
  async findTasks(file: TFile, listItemsCache: ListItemCache[], headings: Headings|undefined): Promise<Task[]> {
    ...
    const tasks: Task[] = listItemsCache
      ...
      .map((lineAndHeading) => {
        // Per-iteration: an untimed task that follows a Day Planner task must
        // NOT inherit the previous task's heading-derived date. Declare the
        // override inside the callback so each task starts with a clean slate.
